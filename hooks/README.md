...
| `resolver/standing-order-guard.sh` | RESOLVER.md universal check #12 (standing-order continuity) | Injects shift-work directive on every `UserPromptSubmit` when `<workspace>/standing-order.json` exists and `active` is true. Path derived from script location (`hooks/resolver/` → `../../`). Ensures standing orders survive loop prompts, stacked messages, and context compaction. Fail-open: silent when no signal file or `active: false`. |
