...
| `resolver/standing-order-guard.sh` | RESOLVER.md universal check #12 (standing-order continuity) | Injects shift-work directive on every `UserPromptSubmit` when `<workspace>/standing-order.json` exists and `active` is true. Path derived from script location (`hooks/resolver/` to `../../`). Ensures standing orders survive loop prompts, stacked messages, and context compaction. Fail-open: silent when no signal file or `active: false`. Craft Agent packages also include `automations-snippet.json` and `standing-order-watchdog.json` as the periodic watchdog layer for sessions that stop after one item while batch work remains. |
