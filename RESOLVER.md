...
| Pre-fix pause reveals an **enforcement contradiction** — the gate blocks its own prerequisites (self-blocking), fires on an innocent command (false positive), creates an inter-rule conflict, or can only be passed by manipulating signal files | **Stop the workaround.** File a ticket with: what you attempted, which gate blocked it, why the block is incorrect (taxonomy class + evidence), and what workaround you would have used. If the original task is operator-declared emergency, note and continue. Otherwise, **pivot to fixing the enforcement gap first** — the broken rule is higher priority than the feature. See `_enforcement-contradiction-rules.md`. Ref: #590, #591, #592, #595. |
