...
- **Fact-sheet basis:** the live-defect ledger; representative entries are `#668 P0-3` (gh stderr suppressed, empty ticket field), `#668 P1-4` (repo root resolved from process CWD, repro executed at `scripts/probe/_common.sh:86`), `#672 P1-7` (documented exit code 3 never emitted; the reservation is at `hooks/create-ticket/scaffold-test-stub.sh:37` and no `exit 3` appears in that file)
...
- The structural-versus-incidental classification required by C-11 returns fewer than 28 of 55 findings as structural. H1 is falsified at its own stated threshold and the epic's premise does not hold; descope to per-defect fixes in bash. This is the weakest of the eight and it is named as such: unlike the other seven it is not a grep, a file count or a named test with an operator-independent answer. "Structural" is defined here only by a parenthetical, no per-finding rule is pre-registered, E-1 records that 40 of the 55 are unclassified, and the classification would be produced by the design ticket of the epic this criterion would kill. The preliminary read is 10 structural to 5 incidental of the 15 classified, so firing requires the remaining 40 to come back more than 20 points worse than the classified head. Treat it as a prompt to re-run the classification under a pre-registered rule, not as a threshold that will trip on its own.

  **What survives descope.** The branch is named as a live outcome, so the constraints have to be sorted against it rather than left to the reader. Six do not survive, because they presuppose a Python artifact that would not exist: C-2 (typed object across module boundaries), C-4 (one dataclass owns the contract), C-8 (the ported suite is a floor, measured by tests failing against bash), C-10 (PORT / REDESIGN / CREATE dispositions, of which PORT is the rewrite), C-12 (the entrypoint checks the Python version) and C-13 (the cutover unit and the interim cross-language contract). Five survive unchanged, because they are about validation and injection rather than language: C-5 (rendering tests read shipped templates from disk), C-11 (every live finding carries a disposition), C-14 (the issue-filer is injected), C-15 (validation is per-sink and per-sink coverage is total) and C-16 (every parsed field reaches a consumer or a documented exclusion). Five survive only in translation and would need restating before they could be enforced: C-1 (one side-effect-free path-validation function that returns or raises), C-3 (a per-tool callable returning a version or nothing), C-6 (install commands as argument lists with a declared privilege tier), C-7 (detection and remediation separated with no import path between them) and C-9 (the contract exercised against a real probe object). The split is 6 / 5 / 5 and not the 8 / 8 a reader might assume from a two-way sort, because the middle category is real: the constraint's intent is language-independent and its stated mechanism is not.
