...
ticket_refs:
  ...
  - siege-analytics/claude-configs-public#684: PR under hostile review
---
...
- **File:** `scripts/probe/_test_probe_common.sh:1-182` (self-contained bash test script; run directly, no harness)
...
| F-N18 | P0 | `_probe_resolve_policy:36` | One `allow\|prompt\|block` token governs both `pip install --user` and `k6.sh:16`'s `sudo gpg` + `sudo tee /etc/apt/sources.list.d/...` + `sudo apt-get install`. No per-tool or per-privilege tier. **Raised P1 -> P0** by the rubric self-falsification pass (round-2 finding 9-3): it shares F-N13's `tool_install_policy: allow` gate with a strictly larger blast radius, so no reading of the rubric rated the two differently. |
...
Unit of analysis, fixed before counting (PR #684 hostile review, finding 1-3): one row of the table above is one finding, and `F-N12b` is a row. That gives **28 new findings** after F-N26 and F-N27 were added by the same review. Severity totals over those 28 rows: **4 P0**, **17 P1**, **7 P2**, produced by `grep -oE '^\| F-N[0-9]+b? \| P[012]' plans/investigate-682-executable-path.md | awk '{print $4}' | sort | uniq -c`. The four P0 findings are F-N1, F-N11, F-N13 and F-N18.

Three revisions preceded this one, and the sequence matters because the current line looks like a revert and is not one. `6 P0, 13 P1, 7 P2` over 25 findings matched neither the table nor itself. `4 P0, 17 P1, 7 P2` followed round 1, correct against the table but produced by a severity rubric that had never been written down; its P0 set was F-N1, F-N11, F-N13, F-N22. `3 P0, 18 P1, 7 P2` followed round-2 finding 9-1, which downgraded F-N22. The present `4 P0, 17 P1, 7 P2` is numerically identical to the post-round-1 line and has a **different P0 set** -- F-N22 left and F-N18 arrived. F-N14 was downgraded per round-1 finding 2-3, F-N22 per round-2 finding 9-1, and F-N18 was raised per finding 9-3, which came out of running the rubric against its own table. The rubric that should have caught all three at authoring time is stated below.
...
**P0.** A defect in the shipped surface whose blast radius is a wrong or dangerous production answer, reachable from a ticket body that a normal author could write, and not observable from the hook's own output. Four findings qualify: filesystem mutation outside the repository root (F-N1), a dead signalling channel (F-N11), a verification step that cannot pass and so reports every successful install as a failure (F-N13), and one policy token governing both a userspace `pip install --user` and a root-privileged apt source addition (F-N18).

**P1.** Either a production defect whose reachability requires a host or environment precondition that this repository does not normally present, or a defect that produces a wrong answer that is observable when it happens. A coverage gap belongs at P1 when it is the reason a P0 is invisible.

**P2.** A defect with no wrong production answer: drift, duplication, naming, or an inert instruction.

Three rules follow. The first two were being violated when the rubric was written. The third came out of running the rubric against its own table afterwards, and it moved an assignment.
...
3. **A supported configuration value is not a bound on reachability.** `tool_install_policy: allow` is one of the three values the skill documents (`skills/tool-availability-probe/SKILL.md:87`). It is not the default -- `:85` sets `block`, annotated "default on shared machines", and `allow` is offered as a commented alternative. But an offered value is a value operators set, and the pre-mortem's own FM-6 blast-radius line describes it as "a token most operators will set to `allow` once and forget". A defect that only bites under `allow` is therefore not bounded by "an environment precondition this repository does not normally present" in the P1 sense; it is bounded by a configuration line the skill invites the operator to write. The P1 bound is for preconditions the operator does not choose -- F-N14's host with neither brew nor apt-get is one, an `allow` policy is not.

**Self-falsification against this table.** A rubric written after the corrections it ratifies is at risk of being reverse-engineered from them: if it only ever agrees with the current table, it is a description rather than a standard. So it was run against all 28 rows as a check. Twenty-seven agreed. One did not. **F-N13 and F-N18 sit behind exactly the same gate** -- `tool_install_policy: allow`, a documented value, reached from an ordinary `Tool:` line in a ticket body -- and were rated P0 and P1. No reading of the rubric supports both at once, so either F-N13 had to fall or F-N18 had to rise. F-N18 rises, for two reasons. Its blast radius is larger than F-N13's: F-N13 files a spurious GitHub issue, F-N18 runs `sudo gpg`, `sudo tee /etc/apt/sources.list.d/k6.list` and `sudo apt-get install` (`scripts/probe/k6.sh:16`) through the `eval "$install_cmd"` at `scripts/probe/_common.sh:166`, and again at `:179` on the `prompt` branch under `TOOL_INSTALL_YES=1`. And the round-1 reviewer said so in the sentence that downgraded F-N14: "the high-severity `eval` exposure stays with F-N18" -- which the table then left at P1, so the reviewer's own reasoning was accepted in prose and dropped in the label. The pre-mortem had the same contradiction from the other side: FM-6 is its highest-scoring failure mode at composite 83, Launch-Blocking, and its blast-radius line calls the path "the single highest-consequence path in the system" -- while the finding underneath it sat at P1. F-N18's precondition is an apt host, which is every Linux CI runner and every Linux development box, and so is not the exotic-host bound that justified F-N14's downgrade. F-N18 moves to **P0**; the totals become `4 P0, 17 P1, 7 P2` and the P0 set becomes F-N1, F-N11, F-N13, F-N18. This is recorded as round-2 finding 9-3 below, found by this check rather than by the reviewer.
...
The same sibling re-reviewed against `cd91ccb` on the two narrow targets named in the round-1 reply. It confirmed the round-1 corrections by re-deriving them: 28 rows with `4 P0, 17 P1, 7 P2`, 55 citation entries with 0 unresolved, the four restated contract sites checked against `scripts/probe/_common.sh:31-34` and `hooks/create-ticket/scaffold-test-stub.sh:215-228`, and both suites re-run green. It reproduced F-N11 independently in a temp repo: a probe writing to stderr and exiting 78 produced `rc=0`, `stub_exists=yes`, `stderr_len=0` at the hook. F-N11 holds at P0. Two new findings from the reviewer, both accepted, plus two the author raised afterwards while writing the `## Severity rubric` section that round 2 motivated.

| Finding | Sev | Substance | Disposition |
|---|---|---|---|
| 9-1 | S2 | F-N22 was P0. It is a coverage gap whose only consequence is that F-N1 is invisible, so P0 double-counted F-N1's blast radius. The fixture also cannot detect the escaped directory even in principle, because `mktemp -d` creates the target before the hook runs. | FIXED. Verified independently by reading `hooks/_test/scaffold_test_stub.test.sh:276-301`: `sibling` is created at `:283` and the assertion at `:297` tests only `-f "$sibling/escaped.py"`. Downgraded to **P1**. Totals at that point became `3 P0, 18 P1, 7 P2` with the P0 set F-N1, F-N11, F-N13; finding 9-3 below moved them again. |
...
| 9-3 | S2 | **Not a reviewer finding.** Raised by running the new `## Severity rubric` against all 28 rows of its own table, which is the check that keeps a post-hoc rubric from being a description of the corrections that produced it. F-N13 (P0) and F-N18 (P1) sit behind the same `tool_install_policy: allow` gate, and F-N18's blast radius is the larger of the two. | FIXED. **F-N18 raised P1 -> P0.** Totals are `4 P0, 17 P1, 7 P2`; the P0 set is F-N1, F-N11, F-N13, F-N18. Citations re-checked at source while writing this: `skills/tool-availability-probe/SKILL.md:87`, `scripts/probe/k6.sh:16`, `scripts/probe/_common.sh:166`. |
| 9-4 | S2 | **Not a reviewer finding.** Re-checking 9-3's citations at source caught a miscitation this artifact and the pre-mortem both carry: `scripts/probe/_common.sh:145` is described in places as "the eval site". `:145` is `probe_run() {`. The two `eval "$install_cmd"` calls are at `:166` (the `allow` branch) and `:179` (the `prompt` branch). The line resolves, and does not say what the sentence attached to it claims. | FIXED here. This is the third instance of round-1 finding 1-2's class in this epic, and the first one found by the author rather than the reviewer. Function-level uses of `:145` for `probe_run` itself are correct and were left alone; only "eval site" phrasings were wrong. The pre-mortem carries the same error at two lines plus a `k6.sh:12` that should be `:16`, and is corrected on its own branch rather than here, per the one-at-a-time rule. The same re-check also caught `scripts/probe/_test_probe_common.sh:1-183` against a 182-line file (`awk 'END{print NR}'`), fixed to `:1-182`. Both this and the `:145` error survived the round-2 audit that reported "0 unresolved" over 55 citation entries, because that audit resolved bare `file:line` citations and did not resolve range upper bounds or check that a resolved line supports its sentence. The citation check is therefore weaker than the artifact claimed for it, which is the honest version of round-1 finding 1-2's lesson. |
| 9-5 | S3 | **Not a reviewer finding on this PR.** 9-3's paragraph above cites FM-6's composite as 84 to argue that the pre-mortem's own scoring contradicted F-N18's P1. The round-1 hostile review on PR #686 established that the pre-mortem carried 85, 84 and 85 for that one score and that the arithmetic under it is 83.0. The number was borrowed from a document that had it wrong in three different ways. | FIXED here: 84 -> 83. The argument is unaffected -- 83 and 84 are both Emergency-stop and both far above every other mode -- which is precisely why nobody re-added it. Recorded rather than silently corrected because it is a fourth instance of importing a number from a neighbouring artifact without checking it, which is what round-1 finding 1-1 was about. |
| 9-6 | S2 | **Not a reviewer finding.** Re-running the citation pass to check 9-5 found that the pass itself does not reproduce. The artifact stated the rule as "every `file.ext[:N[-M]]` in backticks" and reported 105 for this document; that rule as written returns 209, because it counts bare file references carrying no line number. The number that was actually produced required a line number and was computed over this document *and* the self-review together. The claim was true of something, described as something else, and not re-derivable from its own description. | FIXED here: the rule is restated with the line-number requirement explicit, the scope of each figure is named, and both figures are given -- 98 for this artifact, 105 for both. S2 rather than S3 because the pass is the compensating control this artifact offers against the citation-defect class that has now produced four findings (1-2, 9-4, 9-5, and this one), and a control whose output cannot be reproduced from its stated method is not yet a control. The structural fix is to commit the checker as a script instead of describing it in prose, which is a runtime artifact and belongs to its own ticket rather than to this branch. |

The reviewer classified 9-2 as the same class as round-1's 1-2, and that is the right reading. 1-2 was a claim whose `file:line` citation resolved and contradicted it. 9-2 is an evidence label that names a repro that was never run. Both survive a resolution pass, both survive a count audit, and neither is detectable by any check this artifact ran on itself. Four of the eleven findings across the two rounds are severity assignments; the `## Severity rubric` section above is the structural response, and it is a section that should have existed before the first severity was assigned rather than after the third one was corrected.
