...
The citation pass was then re-run with the two gaps 9-4 exposed closed: bare basenames resolved against the tree, and range upper bounds checked rather than only the lower bound. That raises the entry count from 55 to **105** and returns **0 unresolved**. Three shapes resolve to nothing by design and were confirmed one by one rather than suppressed: `PROJECT.md`, whose absence is the substance of F-N3 and is already evidenced inline by `git ls-tree`; `tests/t.py`, `tests/test_ac1_exec_path.py` and `tests/test_ac1_has-hyphen.py`, which are outputs of repros run this session and not files in the tree; and the string `_test_probe_common.sh:1-183` inside finding 9-4, which is that finding quoting the defect it fixed. What the pass still does not do is check that a resolved line supports the sentence citing it. That check has caught three defects in this epic (1-2, 9-4, and the `:145` instances) and remains manual.
