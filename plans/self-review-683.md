...
No P1 rows remain unresolved. L-6 and L-7 are recorded at P0 because both were live in a merged-candidate artifact and both were found by the reviewer rather than by the checks I claimed as compensating controls. L-9 is P0 despite being found by me, because "the author eventually noticed" is not a control, and it is the third instance of the same class.
...
- "106 citations, 0 unresolved" -- walk every `file.ext[:N[-M]]` in backticks across the whole artifact, resolve bare basenames against the tree, assert the file exists and the range's *upper* bound is within its length -> `citations: 106  unresolved: 0`. This supersedes the earlier "55 citations, 0 unresolved", which walked only the `citations` arrays and checked only the lower bound of a range; that weaker pass let `_test_probe_common.sh:1-183` through against a 182-line file. Four non-resolutions are by design and were each confirmed rather than suppressed: `PROJECT.md` (its absence is F-N3), and `tests/t.py`, `tests/test_ac1_exec_path.py`, `tests/test_ac1_has-hyphen.py` (outputs of this session's repros, not tree files). Resolution is still not support: L-6 and L-9 are both citations that resolved and contradicted or failed to support the sentence attached to them
