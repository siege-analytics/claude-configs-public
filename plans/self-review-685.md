...
Hostile-review-artifact: PENDING for #685 itself (see `## Hostile-review-waiver`). The upstream #683 fact sheet this document rests on has had round 1: PR #684, sibling `260831-strong-nova` (GPT-5.5), recommendation Block, 6 findings, all accepted. This document is restated against the corrected fact sheet; see `## Upstream review revision` below.
...
| L-4 | P2 | The document is pinned to `feat/683-investigate-executable-path`, which is unmerged and under hostile review on PR #684. If that review revises severities, four of five Launch-Blocking entries change tier. | partly resolved -- round 1 landed and did revise them. F-N14 went P0 to P1 and two findings were added. The re-weighting rule E-3 stated in advance was applied rather than reconsidered, and no Launch-Blocking tier changed, because tiers are set by blast radius rather than by the severity label on the citation. Round 2 is in flight against F-N11 and F-N22, so the row stays open. |
...
- "322 lines" -- `wc -l < plans/pre-mortem-682-python-rewrite.md` -> `322`
