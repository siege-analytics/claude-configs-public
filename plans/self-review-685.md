...
Hostile-review-artifact: PENDING for #685 itself (see `## Hostile-review-waiver`). The upstream #683 fact sheet this document rests on has had rounds 1 and 2: PR #684, sibling `260831-strong-nova` (GPT-5.5), Block both times, 8 findings, all accepted. This document is restated against the corrected fact sheet; see `## Upstream review revision` below.
Project-contribution: converts the #683 fact sheet from a list of defects into sixteen constraints the step-3 design must satisfy or decline. The specific thing it buys: six of the sixteen failure modes are ones a *correct-looking* Python translation produces, and three of those six are the ones the current green test suite would not catch. Without this document the design ticket's most likely output is a faithful port that passes 19 ported fixtures and ships three P0 defects.
...
| L-4 | P2 | The document is pinned to `feat/683-investigate-executable-path`, which is unmerged and under hostile review on PR #684. If that review revises severities, four of five Launch-Blocking entries change tier. | resolved -- both rounds landed and both revised severities. F-N14 went P0 to P1 in round 1, F-N22 in round 2, and two findings were added. The re-weighting rule E-3 stated in advance was applied both times and no Launch-Blocking tier changed. That the same rule now protects two of my own entries is stated in `## Upstream review revision` as the thing to challenge rather than as a result. |
...
L-6 is P1 and resolved at the document level with its method-level residue stated rather than closed. No other P1 rows remain unresolved.
...
- "0 unresolved citations" -- `python3` resolver over the sixteen `Fact-sheet basis` lines -> `finding IDs checked: 24, file:line checked: 25` / `unresolved: 0`
...
- "5 Launch-Blocking, 3 Mitigate-before-ship, 7 Fast-Follow, 1 Track" -- `awk '/^\| FM-/{split($0,a,"|"); gsub(/ /,"",a[8]); print a[8]}' | sort | uniq -c` -> `7 Fast-Follow, 5 Launch-Blocking, 3 Mitigate-before-ship, 1 Track`. The Launch-Blocking set is the same five it was before the review
- "16 design constraints" -- `grep -c '^- \*\*C-'` -> `16`
- "3 paper tigers, 3 elephants" -- `grep -c '^\*\*PT-'` -> `3`; `grep -c '^\*\*E-'` -> `3`
- "326 lines" -- `wc -l < plans/pre-mortem-682-python-rewrite.md` -> `326`
...
The uncomfortable part is FM-3. Its fact-sheet basis, F-N14, was downgraded from P0 to P1, and FM-3 kept its Launch-Blocking tier. That is either the right answer or motivated reasoning, and the only defence against the second reading is that the rule was written down in E-3 before the review landed: a downgraded P0 re-weights rather than deletes the mode resting on it, because a pre-mortem tier is set by the blast radius of the mode rather than by the severity label on its citation. FM-3's blast radius is spurious public issues on every successful install, and the review did not touch that. The rule fired as written. If round 2 downgrades F-N11 or F-N22 I will apply the same rule and expect to be challenged on it, since applying a rule twice in one's own favour is exactly what the rule was written to prevent and cannot itself distinguish.

Round 2 landed and did exactly that. F-N11 was reproduced and holds at P0, so FM-2 is untouched. F-N22 was downgraded to P1 and FM-8 kept Launch-Blocking under the same rule, for the second time. I am not going to pretend that reads well. What makes it defensible rather than a pattern is that round 2 also strengthened FM-8's independent basis: `test_path_traversal_rejected` creates its escape target with `mktemp -d` at `hooks/_test/scaffold_test_stub.test.sh:283` before the hook runs, so the fixture cannot detect the escaped directory even with the missing assertion added. FM-8's tier rests on that, not on F-N22's label, and C-8 now says the fixture must be rewritten rather than extended. If a reviewer thinks the rule is doing the work instead, that is the finding I want.
