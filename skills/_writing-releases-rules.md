...
- `[`writing-tests`](_writing-tests-rules.md)` writing-tests:3 (skip messages must name the remediation) is the per-skip discipline; writing-releases:2 is the across-PRs trending discipline. Both apply together.
...
- `[`detect-ai-fingerprints`](meta/detect-ai-fingerprints/SKILL.md)` mechanically enforces writing-releases:2 when invoked in `--pr <n>` mode.
