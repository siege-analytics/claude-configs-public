...
Any change that rejects input previously accepted, returns output previously not returned, or removes a public name gets a `### BREAKING` entry. Determined by diffing the public surface against the previous tag. The determination is operator-judgment against an explicit checklist:
...
**Optional mechanical assist:** Python projects may use `griffe check <pkg> --against <ref>` (or an equivalent AST-based public-surface differ) as a CI gate for *signature-level* breaks. Griffe cannot detect behavior changes that preserve signatures (e.g., a validator becoming stricter to reject previously-accepted input) -- those remain operator-judgment. A green griffe run is not evidence that a change is non-BREAKING; a red griffe run IS evidence that BREAKING classification is required. Reference implementation: `siege-analytics/siege_utilities#1191` (CI job authored 2026-08-27). This is the tool-agnostic floor: any Python-static-analysis differ that handles `__all__`, leading-underscore convention, and re-exports through `__init__` satisfies the mechanical-assist role.

The session's worst case (originating incident): a `groupby_agg` validator was added that rejects `median`, `sem`, and other names pandas previously accepted, shipped in a minor release with no BREAKING section. Griffe would have flagged nothing on this incident because the function signature was unchanged -- this is exactly the behavior-not-signature failure mode the rule was written for, and it remains operator-judgment.
