...
- `[`update-ticket`](update-ticket/SKILL.md)` is the platform action surface for comments and fields.
- `[`create-pr`](create-pr/SKILL.md)` owns the In Review transition.
- `[`close-ticket`](close-ticket/SKILL.md)` owns the Done/Closed transition.
- `[`definition-of-done`](_definition-of-done-rules.md)` requires the ticket to reflect the current state before PR and close.
- `[`session-coordination`](_session-coordination-rules.md)` covers agent-to-agent baton and status clarity.
