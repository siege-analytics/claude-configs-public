...
|--------|-----------|------|
| `*.py` with Django imports (`from django`, `django.`) | Django conventions | [`django`](../coding/django/SKILL.md) |
| `*.py` with PySpark imports (`from pyspark`, `import pyspark`) | PySpark patterns | [`spark`](../coding/spark/SKILL.md) |
| `*.py` (general) | Python style | [`python`](../coding/python/SKILL.md) |
| `*.py` library code, especially on re-review / CR feedback touching architecture | Python engineering patterns (DRY, dataclass discipline, interface integrity, runtime types) | [`python-patterns`](../coding/python-patterns/SKILL.md) |
| `try`/`except` on screen, silent-failure patterns, `except Exception: pass` | Python exception discipline | [`python-exceptions`](../coding/python-exceptions/SKILL.md) |
| `*.sql` or SQL in Python strings | SQL conventions | [`sql`](../coding/sql/SKILL.md) |
| SQL with `ST_*` functions, `geometry` / `geography` columns, PostGIS extension | PostGIS patterns | [`postgis`](../coding/postgis/SKILL.md) |
| `*.ts`, `*.tsx` | TypeScript style | `typescript` (planned) |
| `*.tsx`, `*.jsx` with React imports | React patterns | `react` (planned) |
| `*.go` | Go conventions | `go` (planned) |
| `*.py` with pipeline/fetch/job/schedule context, Rundeck YAML, Airflow DAGs | Pipeline job pattern | [`pipeline-jobs`](../coding/pipeline-jobs/SKILL.md) |
