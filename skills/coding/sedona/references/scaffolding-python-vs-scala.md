...
If you don't need that safety, stay with `DataFrame` and the typing difference vanishes. For Spark/Scala typing patterns generally, see [`scala-on-spark`](../../../coding/scala-on-spark/SKILL.md).
...
For Scala-specific patterns around Spark generally (case classes for Dataset[T], encoder gotchas, immutability with `val`), see [`scala-on-spark`](../../../coding/scala-on-spark/SKILL.md).
