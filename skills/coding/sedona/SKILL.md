...
- Single-node datasets (< 10 GB) — use **GeoPandas** ([`geopandas`](../../coding/geopandas/SKILL.md)) or **DuckDB-spatial** ([`duckdb-spatial`](../../coding/duckdb-spatial/SKILL.md)).
- Persistent multi-user query workloads — use **PostGIS** ([`postgis`](../../coding/postgis/SKILL.md)).

## Always-on companions

- [`spark`](../../coding/spark/SKILL.md) — Siege-specific Spark patterns (medallion, catalog rules, transform shape).
- [`scala-on-spark`](../../coding/scala-on-spark/SKILL.md) — when scaffolding is `.scala` or `%scala`.
- [`python`](../../coding/python/SKILL.md) — when scaffolding is PySpark.
- [`data-intensive`](../../shelves/systems-architecture/data-intensive/SKILL.md) — partitioning, replication, batch/stream theory.
