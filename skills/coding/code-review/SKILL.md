...
Before the six-layer human review, run [`detect-ai-fingerprints`](../../meta/detect-ai-fingerprints/SKILL.md) against the diff under review:
...
If `.claude/rules/<topic>.md` files exist, read each one and treat its rules as a project-specific checklist appended to the generic layers below. These rules were promoted from the project's `LESSONS.md` ledger by [`distill-lessons`](../../meta/distill-lessons/SKILL.md) — they encode patterns this codebase has actually been bitten by, so they take priority over generic guidance when they apply.

If the review surfaces a finding that maps to a recurring pattern *not* yet in the ledger, log it via [`lessons-learned`](../../meta/lessons-learned/SKILL.md) before closing the review. That's how the loop closes.
...
- [ ] **[`detect-ai-fingerprints`](../../meta/detect-ai-fingerprints/SKILL.md) ran clean** on the diff (or every reported violation was fixed before review opened)
...
- [ ] Logged any recurring finding to `LESSONS.md` via [`lessons-learned`](../../meta/lessons-learned/SKILL.md)
