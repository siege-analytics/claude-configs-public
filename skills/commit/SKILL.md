...
4. **Run affected tests** (see Affected-tests gate below)
   ...
   6. After all affected tests pass, write `test-gate.json` to the workspace root (or project root) recording test evidence. The `test-guard.sh` hook reads this file at push time. See `[`testing-frameworks`](../_testing-frameworks-rules.md)` testing-frameworks:3 for the schema and `[`testing-frameworks`](../testing-frameworks/SKILL.md)` for framework guidance.
