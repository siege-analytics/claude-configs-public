...
The failure mode this shelf addresses, observed repeatedly across one project's multi-week backfill cycle: correctly-written code, shipped under correctness-shaped review, that assumed a runtime reality which had drifted from the author's mental model. The diff was right relative to imagined state. It was wrong relative to measured state. Multiple distinct production failures in one backfill cycle, each preventable by a one-line measurement command run before the code was written.
...
**This rule supersedes earlier shadow-audit framings.** Downstream forks that previously addressed the cross-project shadow-audit failure mode at edit time (under a `writing-code` numbered rule) should reference authoring-against-state:5 instead; the rule is the same, framed explicitly as a state-measurement act and promoted to this shelf so it composes with the rest of rules 1-5.

**Empirical pattern:** a public symbol added to a package on a shared PYTHONPATH; the pipeline at runtime resolved the package import to a sibling project's older copy lacking the new symbol; the consumer hit `ImportError` hours after the merge. The failure mode is structural: shared import path + multiple copies + no shadow-audit at edit time. The fix is the rule.
...
**Empirical pattern: a Spark/Delta format-drift regression.** The work was authored without inventorying:

- the actual storage format (parquet vs Delta) of the existing target tables -- the epic implied Delta but didn't say it explicitly, the docs were silent, and the running tables were parquet because a prior "no functional change" PR had silently regressed them. A pre-author inventory of `DESCRIBE EXTENDED` output on a representative table would have surfaced `Provider = parquet` and made the regression visible before the smoke test was designed.
- stale mirror stubs in a sibling catalog. A pre-author inventory of `SHOW TABLES IN <mirror-catalog>.<schema>` would have surfaced stubs from prior runs with incompatible partition schemas; without it, the stubs collided with the new DLT registration and failed the smoke at the registration step.
- the Spark Connect server's catalog binding + `spark.sql.extensions` list. A pre-author inventory of `kubectl exec <spark-connect-pod> -- grep -E '(catalog\.<target-catalog>|sql\.extensions)' /opt/spark/conf/spark-defaults.conf` would have surfaced the catalog implementation class actually configured (which lacked Delta) and the extensions list (which also lacked Delta), making the cluster-side gap visible before the pipeline yaml was edited.

Each missing inventory cost one or more smoke-cycle iterations. Each could have been recorded in the parent ticket at the investigation stage, in a `## Pre-author inventory` section, and the next author would have started from that record instead of from a chat history that the next session can't see.

The retrospective on "why did investigation + epic documentation not surface this?" identified four contributing layers: (a) the epic was implicit on the format requirement, (b) the regressing PR's "no functional change" claim went unchallenged in review, (c) the memory entry mis-distilled the regressing PR's commit body, (d) the shelf had rules 1-5 but no rule that mandated reading the inputs + writing findings into the ticket. Rule 6 closes layer (d). The other layers reduce to layer (d) once rule 6 is in place: if the inventory section had been required, the agent would have read the epic (catching layer a), noticed the format drift (catching layer b), and updated the memory (catching layer c) at investigation time.
...
A single multi-cycle arc in which the author of rule 6 itself hit all five rationalizations in five consecutive cycles handled with the diagnose / fix / retrigger loop instead of the inventory loop. The operator called the pattern out three times within the same arc:
...
Empirical pattern: a wrapper DAG was authored to invoke a pre-existing pipeline. The pipeline's library-include glob loaded a sibling utility module that had a bare `from <peer-module> import ...` whose resolution depends on filesystem glob order. The author's pre-flight checked the DAG file but not the wrapped pipeline's load path. The bare-import bug was known from a prior smoke cycle of the same arc -- captured in memory as a follow-up but not brought forward as a blocker for the wrapper-shipping work. The wrapper shipped; the wrapped artifact failed in seconds on its first run.
...
Empirical pattern: in the same arc as the wrapper-authoring incident above, after the first bare-import failure, the right reflex was `grep "^from [a-z_]+ import" <utility-tree>/` to find the other files with the same shape. Instead the response was a 1-line fix on the one that fired, queued for retrigger. Three more retriggers exposed three more instances of the same bug class. The class-scan happened only after operator callout. Total time-to-green was several multiples of the grep-the-class-once approach.
...
Empirical pattern: a memory entry cross-referencing a known-broken module was created on day N during a mid-arc smoke cycle and marked as a follow-up. On day N+1, a wrapper PR shipped DAGs that load the same module's tree at registration time. The known-broken module was not surfaced as a blocker during the wrapper PR's authoring; the DAGs shipped; the wrapper hit the known bug on its first execution. The memory entry's "follow-up" status was treated as "not now" rather than "blocker for any work that triggers this code path."
