...
**Class-membership prose extension.** Class-membership prose is subject to writing-claims:3 when the class is implicitly finite. A claim like "catches try/except optional-import patterns" implicitly claims coverage of ALL shapes in the class "try/except optional-import patterns." If the class is finite and enumerable (which it is for most code-idiom classes — Python has a bounded grammar; SQL injection shapes are a finite catalog; unbounded-I/O call surfaces are a finite library set), the unquantified claim requires either (a) the enumeration per `[rule:writing-rules]` writing-rules:8 in a shape-space coverage table, or (b) a rephrase that names the specific shapes covered rather than the class.
