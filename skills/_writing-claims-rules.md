...
**Class-membership prose extension.** Class-membership prose is subject to writing-claims:3 when the class is implicitly finite. A claim like "catches try/except optional-import patterns" implicitly claims coverage of ALL shapes in the class "try/except optional-import patterns." If the class is finite and enumerable (which it is for most code-idiom classes — Python has a bounded grammar; SQL injection shapes are a finite catalog; unbounded-I/O call surfaces are a finite library set), the unquantified claim requires either (a) the enumeration per `[`writing-rules`](_writing-rules-rules.md)` writing-rules:8 in a shape-space coverage table, or (b) a rephrase that names the specific shapes covered rather than the class.

Signals that trigger this extension:

- "detects <plural noun>" ("detects try/except patterns," "detects race conditions")
- "catches <plural noun>" ("catches SQL injection vectors")
- "enforces <class name>" ("enforces optional-import discipline")
- "covers <plural noun>" ("covers the async surface")
- "handles <class name>" ("handles all import idioms")

The rephrase escape: if the class is genuinely narrow enough that the enumeration is the same as the shape ("catches bare-except handlers" — one shape, no class), the writing-rules:8 table is unnecessary. Rephrase to name the shape directly ("catches `except:` and `except Exception:` handlers"), and writing-claims:3 is satisfied by writing-claims:2's normal grounding.

Worked example of an over-claim caught by this extension: `skills/detect-ai-fingerprints/SKILL.md` at PR #810 time claimed "Detects `try/except ImportError` optional-import patterns" — implicitly covering all shapes in that class. R11 (`plans/r11-scala-skeptic-review.md`) surfaced three INVALIDATING shapes the scanner silently missed (`try/except/else FLAG = True`; `matplotlib.pyplot` prefix pairing; 1-flag/N-import PySpark). The prose class-claim was unquantified; the fixture set enumerated only one variant. Fix: retrofit writing-code:8 with the shape-space coverage table (per writing-rules:8), and either mark the scanner's SKILL.md prose as a documented over-claim pending PR D (#827) OR narrow the SKILL.md prose to the specific shape currently covered.
