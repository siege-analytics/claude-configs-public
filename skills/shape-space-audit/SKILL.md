...
- The diff modifies external-shape-modeling code as defined by `[rule:writing-rules]` writing-rules:5. Concretely: scanners, parsers, linters, hooks that infer semantics from source syntax, config files, CLI arguments, API payloads, schemas, notebooks, or framework conventions. Not Python-specific, not scanner-specific.
- A rule's prose claims to cover "a class of code idioms" (e.g. `[rule:writing-code]` writing-code:8's "optional-import patterns"). Class-membership prose triggers `[rule:writing-claims]` writing-claims:3, which itself triggers this discipline via `[rule:writing-rules]` writing-rules:8's enumeration requirement.
...
**Applied to scanner code:** a coverage claim in SKILL.md prose like "catches try/except optional-import patterns" is a class-membership claim (subject to `[rule:writing-claims]` writing-claims:3). The claim is not backed by a grep for "try/except ImportError" in the scanner source — that only proves the string exists. The claim is backed by an AST-walking check in the scanner that dispatches on `ast.Try` nodes AND a fixture that would regress if the check were removed. The same standard applies at review time: reviewer's coverage-status claim in the writing-code:8 shape table must cite the AST-walking function AND the fixture, not a grep line-count.
...
4. If any variant is silently dropped and SKILL.md's prose claims to cover it, that is a class-membership over-claim per `[rule:writing-claims]` writing-claims:3. File as a review finding, cite the P6 anti-pattern if the coverage claim's original backing was a grep rather than a fixture.
5. Cross-reference every finding with the writing-code:8 shape-space coverage table (per `[rule:writing-rules]` writing-rules:8). Missing row = table gap; row present but marked `covered` when fixture fails = false coverage claim.

## Composition with other rules and skills

- **`[rule:writing-rules]` writing-rules:8** requires this enumeration for any rule whose prose claims to cover a class of code idioms. This skill provides the discipline; the rule provides the enforcement.
- **`[rule:writing-rules]` writing-rules:5** defines external-shape-modeling as never-trivial. This skill's applicability aligns with that definition.
- **`[rule:writing-claims]` writing-claims:3** governs class-membership prose. Unquantified coverage claims like "detects X patterns" are class-membership claims and must be backed by the enumeration this skill produces.
...
- **`[rule:authoring-against-state]` authoring-against-state:6** — the step-2 knowledge-requirements enumeration IS the shape-space enumeration this skill formalizes. Shape-space audit at write time is authoring-against-state's step 2 applied to external-shape-modeling code.
...
Defers to `[rule:output]`. No AI / agent attribution.
