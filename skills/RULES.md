...
| `_authoring-against-state-rules.md` | Data-shape contact (rule 1), config-state contact (rule 2), topology contact (rule 3), plan-shape contact (rule 4), version-resolution contact (rule 5), investigation-as-deliverable -- inventory + falsifiable hypothesis IS the ticket (rule 6, meta-wraps 1-5). Six rules: rules 1-5 specify WHAT to measure; rule 6 specifies HOW the act is performed (read inputs first; apply 1-5; record findings; state a falsifiable hypothesis of what the code will implement) and WHERE the record lives (in the ticket body, not in chat). | Producing a runtime artifact (code, config, command, kick) whose behavior depends on a measurable external state the author has not just inventoried. Sibling to writing-code (verify-before-touching-code) and writing-claims (verify-before-claiming); fills the gap between them. |
