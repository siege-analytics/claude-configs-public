...
| `_authoring-against-state-rules.md` | Data-shape contact (rule 1), config-state contact (rule 2), topology contact (rule 3), plan-shape contact (rule 4), version-resolution contact (rule 5). Five contact-point measurement rules covering the verify-the-external-state-before-authoring-against-it act. | Producing a runtime artifact (code, config, command, kick) whose behavior depends on a measurable external state the author has not just inventoried. Sibling to writing-code (verify-before-touching-code) and writing-claims (verify-before-claiming); fills the gap between them. |
