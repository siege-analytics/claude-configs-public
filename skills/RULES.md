...
| `_writing-rules-rules.md` | Rules that depend on vigilance against a tool default need paired automated enforcement (rule 1), rules surface to the actor who can act on them (rule 2), memory entries are correction layers — not enforcement substitutes (rule 3), every "this doesn't apply" claim requires the same evidence chain as a "this happened" claim (rule 4). Four rule-design rules. | About to add a memory entry / skill / project convention of the form "always do X" or "never do Y"; reviewing an existing rule for promotion or strengthening; filing a project-readiness ticket. |
