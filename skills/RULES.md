...
| `_authoring-against-state-rules.md` | Data-shape contact (rule 1), config-state contact (rule 2), topology contact (rule 3), plan-shape contact (rule 4), version-resolution contact (rule 5), investigation-as-deliverable -- knowledge requirements + inventory + falsifiable hypothesis IS the ticket (rule 6, meta-wraps 1-5). Six rules: rules 1-5 specify WHAT to measure; rule 6 specifies HOW the act is performed (read inputs; enumerate the open questions whose answers determine how the code is written; apply 1-5 plus a broader surface scan to answer them; record findings; state a falsifiable hypothesis of what the code will implement) and WHERE the record lives (in the ticket body, not in chat). | Producing a runtime artifact (code, config, command, kick) whose behavior depends on a measurable external state the author has not just inventoried. Sibling to writing-code (verify-before-touching-code) and writing-claims (verify-before-claiming); fills the gap between them. |
| `_writing-releases-rules.md` | BREAKING in CHANGELOG when public surface changes incompatibly (rule 1), skip-count must not silently trend upward (rule 2), deprecation messages name a removal target (rule 3), tooling deprecation markers require runtime warning (rule 4), verify the published artifact loads in its consumption environment before declaring release done (rule 5). Five release-act rules. | Cutting releases, merging PRs that affect the public surface or the test-suite skip count, adding a deprecation warning or marker, or stamping a final release tag. |
| `_writing-rules-rules.md` | Rules that depend on vigilance against a tool default need paired automated enforcement (rule 1), rules surface to the actor who can act on them (rule 2), memory entries are correction layers -- not enforcement substitutes (rule 3), every "this doesn't apply" claim requires the same evidence chain as a "this happened" claim (rule 4), trivial-claim categories are a controlled vocabulary and "trivial" means "cannot produce a future error" (rule 5), Post-error revision block required when a failure contradicts a documented Assumption (rule 6), rules apply at session-scale and Pairs-with are dependencies (rule 7). Seven rule-design rules. | About to add a memory entry / skill / project convention of the form "always do X" or "never do Y"; reviewing an existing rule for promotion or strengthening; filing a project-readiness ticket. |
...
| `_session-coordination-rules.md` | Eight multi-session coordination rules: processing-state declarations, ping discipline, slow-vs-stuck framing, explicit baton handoff, decision checklist gating, operator control-surface preservation, hub spoke-source selection, and worker/reviewer retirement. |
