...
Run the `[`test-coverage-audit`](../test-coverage-audit/SKILL.md)` procedure to inventory error sites and identify uncovered paths. This gives you the list of `except` blocks, `raise` statements, and error-return paths that need tests.
...
- `[`test-coverage-audit`](../test-coverage-audit/SKILL.md)` — the audit procedure this skill builds on
- `[`writing-tests`](../_writing-tests-rules.md)` writing-tests:5 — the general rule requiring every `except` block to be tested
- `[`hostile-review`](../hostile-review/SKILL.md)` — Priority 4 (SU-4b) uses this skill's output
