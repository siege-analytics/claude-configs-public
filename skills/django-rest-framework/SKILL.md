...
Parent skill: `[`django`](../django/SKILL.md)` -- ORM, models, migrations, settings stratification, factory_boy patterns all apply here unchanged. Read that first.

Complementary rule shelves:
- `[`python`](../_python-rules.md)` -- always-on Python style.
- `[`django-rest-framework`](../_django-rest-framework-rules.md)` -- the numbered principles this skill operationalizes.
- `[`testing-frameworks`](../_testing-frameworks-rules.md)` -- test framework declaration in PROJECT.md; DRF tests declare `rest_framework.test.APITestCase` or `pytest-django` + `APIClient` fixtures.
- `[`writing-releases`](../_writing-releases-rules.md)` -- deprecation and BREAKING discipline for versioned endpoints.
- `[`data-trust`](../_data-trust-rules.md)` -- serializer input is untrusted data; validate at the boundary.
...
The rule of thumb inherited from `[`django`](../django/SKILL.md)`'s "fat models, thin views" doctrine:

- **Model** owns business invariants and domain operations (unchanged from `[`django`](../django/SKILL.md)`).
...
Missing any of 1-5 for a viewset action is a test-gap that composes with `[`writing-tests`](../_writing-tests-rules.md)` writing-tests:5 (every `except` block exercised). DRF handlers translate exceptions to HTTP responses; the handler branch is untested until the failure path is asserted.
