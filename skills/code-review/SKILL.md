...
The scanner catches the four stylistic prose rules (`[rule:writing-prose]` writing-prose:1, :2, :3, :4) plus history-reference detection in code comments (`[rule:writing-code]` writing-code:2), Django ORM kwarg checks for same-file models (`[rule:writing-code]` writing-code:4), silent exception swallowing (`[rule:writing-code]` writing-code:7), optional-import callsite hygiene (`[rule:writing-code]` writing-code:8), silently-dropped defaulted parameters (`[rule:writing-code]` writing-code:9), unbounded blocking I/O (`[rule:writing-code]` writing-code:15), actionable skip messages (`[rule:writing-tests]` writing-tests:3), untested named exception handlers (`[rule:writing-tests]` writing-tests:5), countable claims missing a `Verified-by:` trailer (`[rule:writing-claims]` writing-claims:2), rule citations missing `Rule-executed:` evidence, and deprecation messages without version-or-date plus removal target (`[rule:writing-releases]` writing-releases:3). Surface scanner findings as the first batch of review comments and fix them before opening the human review. The scanner is silent on the rest; those require the judgment layer below. Judgment-only checks worth calling out specifically include `[rule:writing-tests]` writing-tests:4 mock and fixture shape, `[rule:writing-claims]` writing-claims:3 completeness claims outside the countable-claim patterns, and `[rule:writing-releases]` writing-releases:2 skip-count trending.
