...
The sibling rule `[`standing-approval`](_standing-approval-rules.md)` defines how to PARSE a "do X when Y" instruction — what "Y" means, what the gate requires. This rule defines how to RETAIN the parsed instruction over a long trajectory so it actually fires at execution time. Both are needed; either alone is insufficient.

## The four rules

**prospective-memory:1. Repeat the standing instruction's conditional in every covered action's verification block.** When `[`verify-before-execute`](_verify-before-execute-rules.md)` fires on an action covered by a standing instruction (merge, push, send, deploy), add a `**Standing-conditional**` line to the block:
...
The siege existing rule `[`verify-before-execute`](_verify-before-execute-rules.md)` § Evidence requires same-turn tool calls for corrections. This rule extends the principle to standing-conditional checks: same-turn verification or no action. If the verification can't be produced in the same turn, the action does not happen.
...
- **`[`standing-approval`](_standing-approval-rules.md)`** — defines how to PARSE a "do X when Y" instruction (what "ready" means; gate vs. permission vs. timing-deferral). This rule defines how to RETAIN the parsed instruction over a long trajectory. Standing-approval is the linguistic layer; prospective-memory is the attention/memory layer.
- **`[`verify-before-execute`](_verify-before-execute-rules.md)`** — defines the per-action verification block. This rule extends the block with the `Standing-conditional` / `Conditional-met` rows when a standing instruction is in force.
- **`[`definition-of-done`](_definition-of-done-rules.md)`** — defines what "done" means structurally. This rule defines how to keep the done-ness checks live over time.
- **`[`writing-claims`](_writing-claims-rules.md)`** — defines claim-grounding discipline. The `Conditional-met` evidence in this rule's block is the same shape as writing-claims:2's same-turn falsifying check.
