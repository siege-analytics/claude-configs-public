...
The delta is part of the definition of done (`[rule:definition-of-done]` criterion (f)). A contradiction without a filed delta is technical debt.
...
- `[rule:knowledge-base]` -- always-on rules for KB consultation discipline
- `[rule:definition-of-done]` criterion (f) -- KB delta as done criterion (opt-in)
- `[skill:think]` Step 1 -- investigation is where KB consultation happens
- `[skill:investigate]` -- Fact Sheet is the output of investigation; KB tags inform it
