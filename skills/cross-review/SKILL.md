...
- When a coordinator needs an independent reviewer while preserving reviewer/implementer separation under `[`tandem-agent`](../_tandem-agent-rules.md)`
...
reasoning level, source list, rule binding, and the `[`tandem-agent`](../_tandem-agent-rules.md)` role contract. Review sessions use high or
