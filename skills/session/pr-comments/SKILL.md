...
7. **Feed the lessons-learned ledger** — for any human-reviewer comment that flags a recurring pattern (the reviewer says "again," "always," "we keep doing this," or you recognize the pattern from prior PRs), invoke [`lessons-learned`](../../meta/lessons-learned/SKILL.md) to log or bump the entry in `LESSONS.md`. Skip one-off bugs and pure style preferences.
