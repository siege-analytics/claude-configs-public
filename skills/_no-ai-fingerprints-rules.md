...
- `[rule:verify-before-execute]` is the family rule for rules 10 and 11. Verify-before-execute requires same-turn evidence for any side-effecting action; rules 10 and 11 extend the same discipline to claims (rule 10) and to scope-of-fix (rule 11).
...
- `[skill:code-review]` checklist gains rules 7 (tests fail on regression) and 8 (no cargo-culted patterns) as project-default review questions.
