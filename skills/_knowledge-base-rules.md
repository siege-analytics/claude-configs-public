...
The enforcement model follows the established pattern: the skill educates (`[`knowledge-base`](knowledge-base/SKILL.md)`), the rules define vocabulary (this file), and the think-gate-guard Level 3 warns mechanically.
...
Enforcement: think-gate-guard.sh Level 3 warns when `kb.consulted` is missing or false. Judgment-enforced via `[`self-review`](self-review/SKILL.md)` peer review section.
...
Enforcement: judgment-enforced via `[`self-review`](self-review/SKILL.md)` peer review section. The think-gate-guard Level 3 checks that `kb.tags` is non-empty when the signal file has a `kb` section.
...
Part of the definition of done: `[`definition-of-done`](_definition-of-done-rules.md)` criterion (f) (opt-in per project).
...
- `[`knowledge-base`](knowledge-base/SKILL.md)` -- protocol details, platform access, signal file schema
- `[`definition-of-done`](_definition-of-done-rules.md)` criterion (f) -- KB delta as done criterion (opt-in per project)
