...
with a diagnostic naming `[`writing-rules`](../_writing-rules-rules.md)` writing-rules:5 and
this section.

`local-only` deserves specific attention: it IS a legitimate Category
for `## Trivial-against-state declaration` blocks (per
`[`authoring-against-state`](../_authoring-against-state-rules.md)`), where it means "change to a code
...
`[`authoring-against-state`](../_authoring-against-state-rules.md)` step-6 inventory covering the six
...
inventory step IS the enumeration. See `[`writing-rules`](../_writing-rules-rules.md)`
...
Required subsection when the diff modifies external-shape-modeling code (per `[`writing-rules`](../_writing-rules-rules.md)` writing-rules:5 definition: scanners, parsers, linters, hooks that infer semantics from source syntax, config, CLI arguments, API payloads, schemas, notebooks, or framework conventions — not Python-specific, not scanner-specific).

The subsection answers four questions, each requiring same-turn evidence per `[`verify-before-execute`](../_verify-before-execute-rules.md)` and `[`writing-claims`](../_writing-claims-rules.md)` writing-claims:2. Missing subsection when the diff touches ESM code is a self-review block. See `[`shape-space-audit`](../shape-space-audit/SKILL.md)` for the discipline this subsection consumes.
...
- **Shape space** — makes the class-membership claim explicit rather than leaving it implicit in the code. Any downstream `covered` claim (per `[`writing-rules`](../_writing-rules-rules.md)` writing-rules:8) must be measurable against this stated shape space.
...
- If the diff also qualifies as authoring-against-state (per `[`authoring-against-state`](../_authoring-against-state-rules.md)` authoring-against-state:6), the full inventory ships too — shape audit is one layer of that inventory, not the whole thing.
- Trivial-change and Trivial-investigation declarations are INCOMPATIBLE with Adversarial shape audit. External-shape-modeling code is never-trivial per `[`writing-rules`](../_writing-rules-rules.md)` writing-rules:5; if the diff touches ESM code, no Trivial-* declaration is valid, and the Adversarial shape audit subsection is required.
