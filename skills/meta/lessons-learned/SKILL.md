...
| **2 — Project rules** | `<repo>/.claude/rules/<topic>.md` | [`distill-lessons`](../../meta/distill-lessons/SKILL.md) | appears in 2+ projects, or is language/framework-level → Tier 3 |
...
- [`code-review`](../../coding/code-review/SKILL.md) — when reviewing surfaces a finding that "we keep getting this"
- [`coderabbit-response`](../../session/coderabbit-response/SKILL.md) — when a CodeRabbit thread maps to an existing or new pattern
- [`pr-comments`](../../session/pr-comments/SKILL.md) — when a human reviewer flags a recurring concern
- [`wrap-up`](../../session/wrap-up/SKILL.md) — at session end, sweep for findings worth logging
...
```markdown
- **Promoted:** `.claude/rules/<topic>.md` on YYYY-MM-DD by [`distill-lessons`](../../meta/distill-lessons/SKILL.md)
```
...
4. **Append** the entry to `LESSONS.md`. Do not edit the file's header (timestamps, audit cadence) — that's owned by [`rules-audit`](../../meta/rules-audit/SKILL.md).
...
This skill does not perform the promotion — that's [`distill-lessons`](../../meta/distill-lessons/SKILL.md). This skill only logs and surfaces eligibility.
...
- **Editing the audit-timestamp header.** That's [`rules-audit`](../../meta/rules-audit/SKILL.md)'s territory.
