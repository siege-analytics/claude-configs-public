...
| 8 | 2026-05-14 | v2.7.0 negotiation + fix exercise (siege_utilities v3.17.0 verification) | **Three new framework signals + recurrence-2 of LESSON 323a0f5 promotion.** (1) **Recurrence-2 of LESSON 323a0f5 promotion-ready**: independent threshold met (v2.0.0 RULES.md/_coverage.md not in dist; this session's v2.2.0-v2.6.0 untested consumer integration). Operator-stated reinforcement made carve-out (a) single-pass admissible. (2) **Author-self-application during rule drafting (recurrence 1, new pattern)**: sibling caught themselves shipping siege_utilities v3.17.0 to PyPI without verification while drafting writing-releases:5 (the rule that prescribes verification). Distinct from writing-code:3 self-application during drafting (which was rule's content vs another rule); this is rule's content vs its own author's recent action. (3) **Claims-grounded-source-side-only (recurrence 1, new asymmetry pattern)**: agent's "release shipped" claims across v2.2.0-v2.6.0 passed writing-claims:1 source-side (commits, tags, build artifacts) but failed consumer-side. New asymmetry pattern. (4) **Fix-exercise outcome**: sibling's v3.17.0 verification surfaced two real consumer-side bugs (scripts/ namespace pollution; hydra ImportError on optional dep) that source-side CI did NOT catch. Source-side was 100% green (25 jobs, twine check, scanners, full 3.11/3.12/3.13 matrix). siege_utilities issue #496 filed for the scripts/ pollution; v3.17.1 shipped within ~12 minutes of the finding (pyproject.toml exclude scripts*/tests*/docs*); post-publish writing-releases:5 verification on v3.17.1 PASSES (fresh-venv install; site-packages/scripts/ no longer present). (5) **Self-application sanity check**: writing-releases:5 itself was verified post-tag-push by re-syncing the workspace from v2.7.0-flat and confirming the rule appears + scanner exercises correctly. The rule eats its own dogfood at the immediate next opportunity. |
