...
| 8 | 2026-05-14 | v2.7.0 ratification + siege_utilities v3.17.0 verification | writing-releases:5 ratified as Tier-3 rule. Recurrence-2 of LESSON 323a0f5 (v2.0.0 RULES.md/_coverage.md not in dist) cleared independent promotion threshold; operator-stated principle ("verify it, that should be the rule") + sibling's v3.17.0 verification surfacing two real consumer-side bugs (scripts/ namespace pollution + hydra ImportError on optional dep) that source-side CI did NOT catch. siege_utilities issue #496 filed; v3.17.1 patched (pyproject.toml exclude scripts*/tests*/docs*) and shipped to PyPI; post-publish writing-releases:5 verification on v3.17.1 PASSES. Cycle exposed an honest gap: the operator surfaced ("It bothers me that you have been doing all this revision without learning and applying, yourself") that the rule's authors had been authoring discipline as artifact rather than absorbing it — em-dashes the scanner kept catching for both authors retroactively, LESSON 323a0f5 sitting visible through 8 prior releases without being applied, "framework signal recurrence" framing dressing up failures as data. Both authors named specific behavior changes: scan-before-commit (not retroactive); verify-before-announce (not after); call failures failures (not "recurrence 1 of new framework signal"); stop counting release-artifact volume as quality. Three within-session "framework signal" banks initially proposed in this entry's draft (author-self-application during rule drafting, claims-grounded-source-side-only, self-application sanity check) were retracted on review as the same dressing-up-failure-as-data pattern operator surfaced. They are not load-bearing meta-discipline; they were authors banking their own session's failures. Future entries name what was failed, not what new pattern the failure represents. |
