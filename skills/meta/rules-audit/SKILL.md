...
This skill is the **meta-curation** layer of the rules pipeline. The per-tier skills ([`lessons-learned`](../../meta/lessons-learned/SKILL.md) for Tier 1 capture, [`distill-lessons`](../../meta/distill-lessons/SKILL.md) for Tier 1 → Tier 2 promotion) each handle a single transition. This skill owns the **health of the system as a whole**: what's overdue, what's stale, what conflicts, what's ready to promote upstream.
...
1. **On-demand** — `/rules-audit` (or via [`wrap-up`](../../session/wrap-up/SKILL.md)'s 60-day nudge).
...
| Eligible for promotion (per [`distill-lessons`](../../meta/distill-lessons/SKILL.md) thresholds) but not yet promoted | **Promotion overdue** | Run [`distill-lessons`](../../meta/distill-lessons/SKILL.md) for this entry |
...
| Tier-2 rule not loaded by [`code-review`](../../coding/code-review/SKILL.md) (no path-form reference under `.claude/rules/`) | **Wiring gap** | Verify [`code-review`](../../coding/code-review/SKILL.md)'s glob covers the file location |
| LESSONS.md hasn't been touched in 90+ days but the repo has had commits in that window | **Capture gap** | Likely indicates [`lessons-learned`](../../meta/lessons-learned/SKILL.md) isn't being invoked — flag for retrospective |
...
[`wrap-up`](../../session/wrap-up/SKILL.md) reads the `**Last audit:**` line in `LESSONS.md`. If the date is more than 60 days ago, it prints a single-line nudge:
...
- **It doesn't promote entries.** That's [`distill-lessons`](../../meta/distill-lessons/SKILL.md), invoked one entry at a time.
- **It doesn't capture new lessons.** That's [`lessons-learned`](../../meta/lessons-learned/SKILL.md).
...
- **Editing entries during the audit.** The audit is read-only with respect to rules. Findings get written to the audit report; rule edits happen via [`lessons-learned`](../../meta/lessons-learned/SKILL.md) or [`distill-lessons`](../../meta/distill-lessons/SKILL.md) in a separate pass.
