...
For the source ledger spec, see [`lessons-learned`](../../meta/lessons-learned/SKILL.md). For cross-tier hygiene and finding what to promote, see [`rules-audit`](../../meta/rules-audit/SKILL.md).

## When to use

Run after [`rules-audit`](../../meta/rules-audit/SKILL.md) surfaces a "promotion overdue" worklist, or directly when you know a specific entry is ready (e.g., a 🔴 Critical finding logged once that needs immediate codification).

Do **not** use this skill for:
- Promoting from Tier 2 → Tier 3 — that's always a human PR with cited evidence
- Capturing a new lesson — that's [`lessons-learned`](../../meta/lessons-learned/SKILL.md)
- Auditing the system as a whole — that's [`rules-audit`](../../meta/rules-audit/SKILL.md)
...
If the rule is not code-checkable, ensure it's wired into [`code-review`](../../coding/code-review/SKILL.md) by updating the project's `.claude/rules/<topic>.md` (which `code-review` reads at the top of every review).
...
Project-local Tier-2 rules for `<topic>`. Distilled from `LESSONS.md` entries by [`distill-lessons`](../../meta/distill-lessons/SKILL.md). Loaded by [`code-review`](../../coding/code-review/SKILL.md) at the start of every review.

For the promotion process, see [`distill-lessons`](../../meta/distill-lessons/SKILL.md). For the source ledger, see [`lessons-learned`](../../meta/lessons-learned/SKILL.md).
