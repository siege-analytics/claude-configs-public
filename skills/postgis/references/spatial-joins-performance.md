...
If your spatial join is a single query against parquet files, no Postgres in the picture, consider DuckDB-spatial — see [`duckdb-spatial`](../../duckdb-spatial/SKILL.md).

If your data is bigger than one Postgres node can hold or the join exceeds memory, move to Sedona on Spark — see [`sedona`](../../sedona/SKILL.md).
