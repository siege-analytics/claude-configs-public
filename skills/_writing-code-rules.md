...
**Narrow `except` that doesn't catch is the same failure shape.** `except SomeSpecificClass: warn+return False` is silent-swallow if `SomeSpecificClass` is not actually the class the wrapped operation raises. The handler reads like a defensive catch but the exception bypasses it entirely -- same outcome as `except: pass`, different syntax. Before writing a narrow `except`, verify the exception class against the actual `raise` path of the wrapped operation. Substrate-specific gotchas: client-server bridges (Spark Connect, gRPC, JDBC) often have parallel exception hierarchies where the client-side wrapper raises a different class than the local-only path. Grep the library docs or run the failing call once to capture the actual `type(exc).__module__ + '.' + type(exc).__name__` before relying on a narrow class. The Spark Connect case that motivated this clarification: agents wrote `except pyspark.errors.exceptions.captured.AnalysisException` thinking it caught Spark Connect's analysis errors; Connect actually raises a different class, the narrow catch never fires, and the exception escapes module-load.
...
**Shape-space coverage table.** Per `[`writing-rules`](_writing-rules-rules.md)` writing-rules:8, rules that describe a class of code idioms must enumerate the covered shape space. The class writing-code:8 targets is "optional-import patterns"; the shape space is enumerated here.

**Coverage-status semantics (read this before interpreting the table):** an entry may be marked `covered` only when the coverage claim is backed by BOTH (a) an AST- or control-flow-aware code path in the scanner AND (b) at least one executable fixture that regresses if the code path is removed. Context-free grep for the pattern in scanner source does NOT satisfy `covered`; it proves the string exists, not that the shape is handled. See `[`shape-space-audit`](shape-space-audit/SKILL.md)`'s named P6 anti-pattern for the underlying discipline.
...
- Flipping a row from `not-covered` to `covered`: requires (a) a scanner code path in the same PR AND (b) an executable fixture in the same PR AND (c) reviewer sign-off per `[`hostile-review`](hostile-review/SKILL.md)` Category 10c (prose-vs-implementation audit). All three; no exceptions.
...
**Family-level consistency, not just per-function.** Operations across a family (same import path, same decorator, same call shape, same role in the system -- e.g. all materialized-view guards, all retry wrappers, all snapshot writers) must fail the same way. Inconsistency across the family IS a contract violation even if each individual operation has a defensible default. The Spark Connect guard family that motivated this clarification had five guards within one session: two defaulted to True on session-None, one defaulted to False, two used a narrow except that didn't fire -- five members of the same family, four different failure shapes. Per writing-claims:1 + writing-rules:7, the family-level scan happens BEFORE the second-instance fix lands, not after the fifth.
...
**writing-code:18. "Fix and keep moving" requires investigate/pre-mortem artifacts first.**

The default bias toward forward motion -- documented unblockers and routine next-steps -- is correct. Just do them. But "just do them" applies AFTER the investigate and pre-mortem artifacts exist for the relevant domain. Without those artifacts, "fix and keep moving" is "fix without evidence and hope the class is closed." The investigate Fact Sheet establishes what the domain actually contains; the pre-mortem classifies what can go wrong. Forward motion without those two artifacts is not efficiency -- it is skipping the gates that prevent the fix from being one of N identical fixes across a session.
