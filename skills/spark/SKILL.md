...
- [`shelves--effective-java`](../shelves/languages/effective-java/SKILL.md) -- equals/hashCode for Dataset[T], immutability, exception handling in UDFs.
- [`shelves--effective-kotlin`](../shelves/languages/effective-kotlin/SKILL.md) -- null-safety idioms (≈ Scala Option/Either).
- [`shelves--systems-architecture`](../shelves/systems-architecture/SKILL.md) (`skills/shelves/systems-architecture/data-intensive/SKILL.md`) -- partitioning, shuffle, replication theory.

Always-on: [`jvm`](../_jvm-rules.md) is loaded when Spark/JVM code is touched.
For Scala notebooks specifically, see also [`scala-on-spark`](../scala-on-spark/SKILL.md).
