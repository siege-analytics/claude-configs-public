---
description: Always-on Django REST Framework standards. Apply when writing or reviewing Python code that imports from rest_framework.*. Stacks on top of [`python`](_python-rules.md) and `_django-rules.md` (planned).
---

# Django REST Framework Standards

Apply these principles to all code that imports from `rest_framework.*`. Layers on top of `[`python`](_python-rules.md)` for Python style and ``_django-rules.md` (planned)` for ORM, model, and settings conventions. This shelf covers the DRF-specific surface: serializers, viewsets, permissions, pagination, throttling, testing, versioning, and schema documentation.
...
- Fixtures via `pytest-django` + `factory-boy`, not `fixtures/*.json`. See `[`django`](django/SKILL.md)` for the factory pattern; DRF tests use the same factories to build the request payload
...
- Deprecation of a version follows `[`writing-releases`](_writing-releases-rules.md)` writing-releases:1 (BREAKING in the changelog) and writing-releases:3 (deprecation messages name a removal target). A v1 endpoint slated for removal has both an `X-API-Deprecated` header on responses and a documented removal version
