...
These rules enforce consistent test framework usage across projects. They complement the test-quality rules in `[rule:writing-tests]` (which govern _how_ to write tests) with infrastructure rules governing _which frameworks_ to use and _how evidence is verified_.

The enforcement model follows the branch-guard pattern: the skill educates (`[skill:testing-frameworks]`), the rules define vocabulary (this file), and the hook blocks mechanically (`hooks/git/test-guard.sh`).
...
Enforcement: judgment-enforced via `[skill:code-review]` until a mechanical scanner lands. The code-review skill checks new `test_*.py` / `*.spec.ts` / `*.test.ts` files against the declared framework in `PROJECT.md`.
...
| Rule | Enforcement |
|---|---|
| testing-frameworks:1 (declare frameworks) | `bin/build.py` validates `testing:` schema in PROJECT.md for active projects; `[skill:pre-work-check]` warns on missing section |
| testing-frameworks:2 (use declared framework) | Judgment-enforced via `[skill:code-review]`; scanner deferred |
| testing-frameworks:3 (record and verify evidence) | `hooks/git/test-guard.sh` blocks pushes without evidence; `[skill:commit]` step 4.6 writes `test-gate.json` |
