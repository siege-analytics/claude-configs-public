...
- data validation, caches, persisted API results, materialized views, or retry/failure semantics require [`shelves--data-intensive`](../shelves/systems-architecture/data-intensive/SKILL.md). Apply DDIA's "data outlives code" principle: identify system of record vs derived data, freshness/consistency promises, partial-failure behavior, and whether bad data can persist after the code is fixed.
- geocoding, coordinates, CRS, spatial caches, and geometry operations require [`shelves--geospatial`](../shelves/geospatial/SKILL.md) in addition to [`shelves--data-intensive`](../shelves/systems-architecture/data-intensive/SKILL.md) when persisted or derived data is involved.
- API/public-surface promotions require [`code-review`](../code-review/SKILL.md)'s public-contract checks: degraded optional dependency behavior, star import/introspection behavior, catchable documented exception types, and explicit tests for those promises.
...
Fires whenever the target of hostile review is scanner/parser/linter/hook code — i.e. external-shape-modeling code per `[`writing-rules`](../_writing-rules-rules.md)` writing-rules:5. Three sub-categories; each is a required audit lens when Category 10 applies. See `[`shape-space-audit`](../shape-space-audit/SKILL.md)` for the discipline this category consumes and `[`writing-rules`](../_writing-rules-rules.md)` writing-rules:8 for the enforcement rule Category 10 checks against.
...
For scanners with multiple related detectors (e.g. `[`writing-code`](../_writing-code-rules.md)` writing-code:7 silent-swallow + writing-code:8 optional-import-guard), constructs inputs where one detector's guard clause could silently mask another detector's finding.
...
**Composition with the P6 anti-pattern:** grepping SKILL.md for coverage claims is fine (that's discovery). The audit becomes evidence when (a) and (b) are verified per claim — not when a grep count is reported as coverage. See `[`shape-space-audit`](../shape-space-audit/SKILL.md)`'s named P6 pattern: context-free grep is not shape evidence.

**Applied to the writing-code:8 shape-space table** (per `[`writing-rules`](../_writing-rules-rules.md)` writing-rules:8): the table's `Coverage status` column is the mechanical output of Category 10c. A row marked `covered` claims both (a) and (b) hold; the reviewer's job is to falsify that claim by finding a real code shape the row's example represents but the scanner silently drops.
