...
2. Cite applicable general guides and shelves in the review artifact/comment. For geocoding, caches, data validation, persisted API responses, and derived data, cite [`shelves--data-intensive`](shelves/systems-architecture/data-intensive/SKILL.md) and apply DDIA's system-of-record vs derived-data lens. For coordinates/CRS/spatial helpers, also cite [`shelves--geospatial`](shelves/geospatial/SKILL.md).
