...
- [`shelves--effective-java`](../shelves/languages/effective-java/SKILL.md) -- `equals`/`hashCode`, immutability, exception handling, generics. Items map almost 1:1 to Scala case classes and `Either`-shaped APIs.
- [`shelves--effective-kotlin`](../shelves/languages/effective-kotlin/SKILL.md) -- null-safety idioms (Scala `Option`/`Either` ≈ Kotlin nullable + `let`), DSLs, scope functions.
- [`shelves--systems-architecture`](../shelves/systems-architecture/SKILL.md) (`skills/shelves/systems-architecture/data-intensive/SKILL.md`) -- partitioning, replication, batch/stream, the *why* behind Spark's cost model.
