...
6. **Move each referenced ticket to In Review** per `[`ticket-lifecycle`](../_ticket-lifecycle-rules.md)` (or add `Status: In Review` comment when no mutable status field is available)
