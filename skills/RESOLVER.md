...
| "Review this PR", "review this diff" | [`code-review`](coding/code-review/SKILL.md) |
| Library code architecture — DRY, dataclass discipline, interface integrity, runtime types | [`python-patterns`](coding/python-patterns/SKILL.md) |
| "Why is this try/except too broad?", silent-failure patterns, `except Exception: pass` | [`python-exceptions`](coding/python-exceptions/SKILL.md) |
| Django models, views, forms, migrations, settings | [`django`](coding/django/SKILL.md) |
| Data pipeline, scheduled job, Rundeck YAML, Airflow DAG | [`pipeline-jobs`](coding/pipeline-jobs/SKILL.md) |
| PySpark DataFrame work, tuning, shuffle / skew | [`spark`](coding/spark/SKILL.md) |
| Scala on Spark / Databricks (`.scala`, `%scala`, `Dataset[T]`) | [`scala-on-spark`](coding/scala-on-spark/SKILL.md) |
| SQL query structure, joins, window functions, Postgres performance | [`sql`](coding/sql/SKILL.md) |
| PostGIS — ST_* functions, spatial indexes, spatial joins | [`postgis`](coding/postgis/SKILL.md) |
| GeoPandas + Shapely — `import geopandas`, `gpd.`, `.sjoin`, raw `from shapely.geometry import` | [`geopandas`](coding/geopandas/SKILL.md) |
| Apache Sedona — `from sedona`, `SedonaContext`, ST_* in Spark SQL, `%scala` Sedona | [`sedona`](coding/sedona/SKILL.md) |
| DuckDB-spatial — `import duckdb` + `INSTALL spatial` / `LOAD spatial` / `ST_Read` (single-node SQL on Parquet, GDAL-less) | [`duckdb-spatial`](coding/duckdb-spatial/SKILL.md) |
| QML component review — properties-in / signals-out, MuseScore plugins, Qt Quick decomposition | [`qml-component-review`](coding/qml-component-review/SKILL.md) |

### Analysis

| Trigger | Skill |
|---|---|
| Geospatial data — pick engine + GDAL-availability path, cross-engine principles | [`spatial`](analysis/spatial/SKILL.md) (router; dispatches to `coding/{postgis,geopandas,sedona,duckdb-spatial}/`) |
...
| Trigger | Skill |
|---|---|
| Renaming a public function, changing a default, dropping a column | [`library-api-evolution`](maintenance/library-api-evolution/SKILL.md) |
| Redundant docs across repos, "same thing in five places" | [`consolidate`](maintenance/consolidate/SKILL.md) |

### Git workflow

| Trigger | Skill |
|---|---|
| Create a new branch | [`branch`](git-workflow/branch/SKILL.md) |
| Commit changes | [`commit`](git-workflow/commit/SKILL.md) |
| Open a PR | [`create-pr`](git-workflow/create-pr/SKILL.md) |
| Merge a PR | [`merge`](git-workflow/merge/SKILL.md) |
| Protect `develop` / `main` | [`develop-guard`](git-workflow/develop-guard/SKILL.md) |

### Session

| Trigger | Skill |
|---|---|
| CodeRabbit or other bot review on a PR | [`coderabbit-response`](session/coderabbit-response/SKILL.md) |
| Inline PR discussion threads | [`pr-comments`](session/pr-comments/SKILL.md) |
| End-of-session summary, close the loop | [`wrap-up`](session/wrap-up/SKILL.md) |

### Planning

| Trigger | Skill |
|---|---|
| Create a Linear / Jira / GitHub issue | [`create-ticket`](planning/create-ticket/SKILL.md) |
| Update ticket status | [`update-ticket`](planning/update-ticket/SKILL.md) |
| Close a ticket | [`close-ticket`](planning/close-ticket/SKILL.md) |
| "What should I work on next?", opportunity surfacing | `im-feeling-lucky` (planned) |
| Starting work on a ticket — claim it, mark in-progress, branch, then code | [`pre-work-check`](planning/pre-work-check/SKILL.md) |

### Documentation

| Trigger | Skill |
|---|---|
| Central docs consolidation across repos | [`cascading-documentation`](documentation/cascading-documentation/SKILL.md) |
| Notion knowledge base | [`notion-knowledge-base`](documentation/notion-knowledge-base/SKILL.md) |

### Thinking

| Trigger | Skill |
|---|---|
| Break down a problem, decision framework | [`think`](thinking/think/SKILL.md) |

### Meta

| Trigger | Skill |
|---|---|
| "Create a skill", "audit skills", "fix this skill" | [`skillbuilder`](meta/skillbuilder/SKILL.md) |
...
| Trigger | Skill |
|---|---|
| Databricks workspace, jobs, DLT, Delta, Unity Catalog, Photon, liquid clustering | [`databricks`](infrastructure/databricks/SKILL.md) |
| Unity Catalog permissioning specifics (standalone) | [`unity-catalog`](infrastructure/unity-catalog/SKILL.md) |
| Operating shared infra — cyberpower, K8s, Rundeck — process and concurrency limits | [`ops`](infrastructure/ops/SKILL.md) |
